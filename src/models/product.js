...
module.exports = {
  ...
  searchProduct: (seacrhName) => {
    return new Promise((resolve, reject) => {
      ...
      connection.query('SELECT product.product_code as id, product.name as name, product.desc as description, product.photo as image, product.price as price,  product.stock, category.name, category.id as category,  product.data_added, product.data_updated from product INNER JOIN category WHERE product.category = category.id AND product.name LIKE "%' + name + '%"', (error, result) => {
        if (error) reject(new Error(error))
